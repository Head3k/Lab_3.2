...
int encode(uint32_t code_point, CodeUnit* code_units)
{
	...
	else
	{
		...
		int mask = 8 - code_units->length; //кол-во бит, на которое нужносдвинуть для первого байта
...
{
	for (size_t i = code_unit->length; i > 0; i--) //побайтово записываем вфайл
	{
...
int read_next_code_unit(FILE * in, CodeUnit * code_units)
{
	...
	if (n[0] >> 7 == 0) //если служебное число - 0
	{
		...
		for (int i = 5; i >= 3; i--)
		{
			if (n[0] >> (8 - i) == (pow(2, i) - 2)) // если маска равнастепени
			{ // 110=(2^3)-2 (6=6)
			int count = 0;
			uint8_t y = n[0] >> (8 - i + 1);
